...
@section('title', 'Manajemen Data Guest')
        ...
        <div class="panel-body" style="border-top: 1px solid #eee; padding:15px; background:white;">
            <div class="row" style="margin-right:-15px; margin-left:-15px;">
                <div class="col-md-12">
                    @if ($message   = Session::get('success'))
                        ...
                        @else
                            <div class="alert alert-success alert-block" id="keterangan">
                                <button type="button" class="close" data-dismiss="alert">×</button>
                                <strong><i class="fa fa-info-circle"></i>&nbsp;Perhatian: </strong> Berikut adalah data guest aplikasi arsip berkas universitas bengkulu, silahkan tambahkan admin baru jika dibutuhkan !!
                            </div>
                    @endif
                </div>
                <div class="col-md-12">
                    <a onclick="tambahAdmin()" class="btn btn-primary btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-user-plus"></i>&nbsp; Tambah guest</a>
                </div>
                <div class="col-md-12">
                    <table class="table table-striped table-bordered" id="table" style="width:100%;">
                        ...
                        <tbody>
                            ...
                            @foreach ($guests as $guest)
                                <tr>
                                    ...
                                    <td style="text-align:center;"> {{ $guest->email }} </td>
                                    <td style="text-align:center;"> {{ $guest->nm_unit }} </td>
                                    <td style="text-align:center;">
                                        <a onclick="ubahPassword( {{ $guest->id }} )" class="btn btn-primary btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-key"></i></a>
                                    </td>
                                    <td style="text-align:center;">
                                    ...
                                    <td style="text-align:center;">
                                        <a onclick="ubahData( {{ $guest->id }} )" class="btn btn-primary btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-edit"></i></a>
                                        <a onclick="hapusData( {{ $guest->id }} )" class="btn btn-danger btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                        ...
                        <div class="modal fade" id="formubahpassword" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            ...
                            <div class="modal-content">
                                <form action=" {{ route('administrator.guest.update_password') }} " method="POST">
                                    {{ csrf_field() }} {{ method_field('PATCH') }}
                                    <div class="modal-header">
                                        <p style="font-size:15px; font-weight:bold;" class="modal-title"><i class="fa fa-key"></i>&nbsp;Form Ubah Password guest</p>
                    ...
                    <div class="modal fade" id="modalubah" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form action=" {{ route('administrator.guest.update') }} " method="POST">
                                {{ csrf_field() }} {{ method_field('PATCH') }}
                                <div class="modal-header">
                                    <p style="font-size:15px; font-weight:bold;" class="modal-title"><i class="fa fa-key"></i>&nbsp;Form Ubah Data guest</p>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="unit_id_edit" class="form-control" id="unit_id_edit" required>
                ...
                <div class="modal fade modal-danger" id="modalhapus" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form action=" {{ route('administrator.guest.delete') }} " method="POST">
            ...
            <div class="modal fade" id="modaltambah" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action=" {{ route('administrator.guest.post') }} " method="POST">
                        {{ csrf_field() }} {{ method_field('POST') }}
                        <div class="modal-header">
                            <p style="font-size:15px; font-weight:bold;" class="modal-title"><i class="fa fa-key"></i>&nbsp;Form Tambah Data guest</p>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="unit_id" class="form-control @error('unit_id') is-invalid @enderror" id="unit_id">
                                            <option disabled selected>-- pilih unit --</option>
                                            @foreach ($units as $unit)
                                                <option {{ old('unit_id') == $unit->id ? 'selected' : '' }} value="{{ $unit->id }}">{{ $unit->nm_unit }}</option>
                                            @endforeach
                                        </select>
                                        <small class="form-text text-danger">{{ $errors->first('unit_id') }}</small>
                                    </div>
...
@push('scripts')
    <script>
        ...
        function ubahData(id){
            $.ajax({
                url: "{{ url('administrator/manajemen_guest') }}"+'/'+ id + "/edit",
                type: "GET",
                dataType: "JSON",
