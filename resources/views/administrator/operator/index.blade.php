...
@section('title', 'Manajemen Data Operator')
        ...
        <div class="panel-body" style="border-top: 1px solid #eee; padding:15px; background:white;">
            <div class="row" style="margin-right:-15px; margin-left:-15px;">
                <div class="col-md-12">
                    @if ($message   = Session::get('success'))
                        ...
                        @else
                            <div class="alert alert-success alert-block" id="keterangan">
                                <button type="button" class="close" data-dismiss="alert">×</button>
                                <strong><i class="fa fa-info-circle"></i>&nbsp;Perhatian: </strong> Berikut adalah data operator aplikasi arsip berkas universitas bengkulu, silahkan tambahkan admin baru jika dibutuhkan !!
                            </div>
                    @endif
                </div>
                <div class="col-md-12">
                    <a onclick="tambahAdmin()" class="btn btn-primary btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-user-plus"></i>&nbsp; Tambah Operator</a>
                </div>
                <div class="col-md-12">
                    <table class="table table-striped table-bordered" id="table" style="width:100%;">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align:center">Nama Operatoe</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($operators as $operator)
                                <tr>
                                    ...
                                    <td style="text-align:center;"> {{ $operator->nm_user }} </td>
                                    <td style="text-align:center;"> {{ $operator->email }} </td>
                                    <td style="text-align:center;"> {{ $operator->nm_unit }} </td>
                                    ...
                                    <td style="text-align:center;">
                                        @if ($operator->status == "aktif")
                                            <form action="{{ route('administrator.operator.nonaktifkan_status',[$operator->id]) }}" method="POST">
                                                {{ csrf_field() }} {{ method_field('PATCH') }}
                                                <button type="submit" class="btn btn-danger btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-thumbs-down"></i></button>
                                            </form>
                                            @else
                                            <form action="{{ route('administrator.operator.aktifkan_status',[$operator->id]) }}" method="POST">
                                                {{ csrf_field() }} {{ method_field('PATCH') }}
                                    ...
                                    <td style="text-align:center;">
                                        <a onclick="ubahData( {{ $operator->id }} )" class="btn btn-primary btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-edit"></i></a>
                                        <a onclick="hapusData( {{ $operator->id }} )" class="btn btn-danger btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                        ...
                        <div class="modal fade" id="formubahpassword" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            ...
                            <div class="modal-content">
                                <form action=" {{ route('administrator.operator.update_password') }} " method="POST">
                                    {{ csrf_field() }} {{ method_field('PATCH') }}
                                    <div class="modal-header">
                                        <p style="font-size:15px; font-weight:bold;" class="modal-title"><i class="fa fa-key"></i>&nbsp;Form Ubah Password Operator</p>
                    ...
                    <div class="modal fade" id="modalubah" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form action=" {{ route('administrator.operator.update') }} " method="POST">
                                {{ csrf_field() }} {{ method_field('PATCH') }}
                                <div class="modal-header">
                ...
                <div class="modal fade modal-danger" id="modalhapus" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form action=" {{ route('administrator.operator.delete') }} " method="POST">
            ...
            <div class="modal fade" id="modaltambah" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action=" {{ route('administrator.operator.post') }} " method="POST">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="unit_id" class="form-control @error('unit_id') is-invalid @enderror" id="unit_id">
                                            <option disabled selected>-- pilih unit --</option>
                                            @foreach ($units as $unit)
                                                <option {{ old('unit_id') == $unit->id ? 'selecte' : '' }} value="{{ $unit->id }}">{{ $unit->nm_unit }}</option>
                                            @endforeach
                                        </select>
                                        <small class="form-text text-danger">{{ $errors->first('unit_id') }}</small>
                                    </div>
...
@push('scripts')
    <script>
        ...
        function ubahData(id){
            $.ajax({
                url: "{{ url('administrator/manajemen_operator') }}"+'/'+ id + "/edit",
                type: "GET",
                dataType: "JSON",
